...
Item {
    ...
    */
    ...
    /*!
        ...
        The range is from \c 0 degrees to \c 360 degrees.
    */
    readonly property real angle: internal.mapFromValue(minValue, maxValue, startAngle, endAngle, value)
    ...
    */
    ...
    property real centerSnapThreshold: 3.0 // Puedes ajustar este valor
    ...
    QtObject {
        ...
        //     // Mantener el ángulo tal como está (sin normalizar)
        //     var start = control.startAngle;
        //     var end = control.endAngle;

        //     // Corregir si end < start (es decir, si pasa por el 0)
        //     var inRange = isAngleBetween(angleVal, start, end);

        //     if (inRange) {
        //         internal.setUpdatedValue = true;
        //         internal.angleProxy = Qt.binding(function() { return angleVal; });
        //         if (control.snap) {
        //             var mappedValue = internal.mapFromValue(start, end, control.minValue, control.maxValue, angleVal);
    ...
    function _updateValueFromAngle(angleDeg) {
        // Añadimos el anclaje al centro si el ángulo está dentro del umbral
        if (control.centerSnapThreshold > 0 && Math.abs(angleDeg) < control.centerSnapThreshold) {
            angleDeg = 0; // Forzamos el ángulo a 0
        }

        // El resto de la lógica permanece igual
        if (isAngleBetween(angleDeg, control.startAngle, control.endAngle)) {
            var newValue = internal.mapFromValue(control.startAngle, control.endAngle, control.minValue, control.maxValue, angleDeg);

            if (control.snap) {
                var snapped = Math.round(newValue / control.stepSize) * control.stepSize;
                control.value = Math.max(control.minValue, Math.min(control.maxValue, snapped));
            } else {
    ...
    MouseArea {
        ...
        onClicked: {
            ...
            var clickedDistance = Math.pow(mouseX - internal.centerPt.x, 2) + Math.pow(mouseY - internal.centerPt.y, 2);
            var innerRadius2 = innerRadius * innerRadius;
            var outerRadius2 = outerRadius * outerRadius;

            if (clickedDistance <= outerRadius2 && clickedDistance > innerRadius2) {
                // Calcula el ángulo, ajusta el origen a la vertical (12 en punto) y normaliza
                ...
                _updateValueFromAngle(angleDeg);
            }
        }
    }
    ...
    //             var angleDeg = Math.atan2(mouseY - internal.centerPt.y, mouseX - internal.centerPt.x) * 180 / Math.PI;
    //             angleDeg += 90;
    //             angleDeg = normalizeAngle(angleDeg);  // <- muy importante
    ...
    Item {
        ...
        MouseArea {
            ...
            function getAngleFromMouse() {
                var handlePoint = mapToItem(control, mouseX, mouseY);
                var angleDeg = Math.atan2(handlePoint.y - internal.centerPt.y, handlePoint.x - internal.centerPt.x) * 180 / Math.PI + 90;
                angleDeg = normalizeAngle(angleDeg);
                _updateValueFromAngle(angleDeg);
            }

            onPositionChanged: getAngleFromMouse()
            onClicked: getAngleFromMouse()
        }
